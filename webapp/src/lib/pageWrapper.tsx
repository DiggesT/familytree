...
type PageWrapperProps<TProps, TQueryResult extends QueryResult | undefined> = {
...
const PageWrapper = <TProps extends Props, TQueryResult extends QueryResult | undefined = undefined>({
  ...
  useEffect(() => {
    if (redirectNeeded) {
      void navigate(getAllMembersRoute(), { replace: true })
    }
  ...
  if (checkExists) {
    const notExist = !checkExists(helperProps)
    if (notExist) {
  ...
  try {
    ...
    const exactTitle = isTitleExact ? calculatedTitle : `${calculatedTitle} - Family Tree`
    return (
      <>
        <Helmet>
